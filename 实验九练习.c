...
int main(){
	...
	printf("Enter students'number,name,math grades,English grades,Chinses grades:\n");
	for(i=0; i<5; i++){
		scanf("%d %s %d %d %d",&(stud[i].Num),stud[i].Name,&(stud[i].Math),&(stud[i].English),&(stud[i].Chinses));
	    stud[i].averageValue=(stud[i].Math+stud[i].English+stud[i].Chinses)/3;
	}
		
	fputs("NUMBER  NAME  MATH  ENGLISH  CHINSES  AVERAGENUM\n", inFile);
	for(i=0; i<5; i++){
		fprintf(inFile, "%4d   %4s   %3d   %4d   %7d   %6.2f\n", stud[i].Num,stud[i].Name,stud[i].Math,stud[i].English,stud[i].Chinses,stud[i].averageValue);
	}
